...
def free_memory(memory_required: float, device: torch.device, keep_loaded: list["LoadedModel"] = [], discard: bool = False):
    ...
    for x in sorted(can_unload):
        ...
        if current_loaded_models[i].model_unload(memory_to_free, unpatch_weights=not discard):
...
def unload_all_models(discard: bool = False):
    """Unload everything.

    With discard=True the models are being thrown away, so their weights are not
    copied back to the offload device first - that copy is wasted work, and on
    float8 diffusion models it crashes the process inside torch's CUDA->CPU copy.
    """
    free_memory(1e30, get_torch_device(), discard=discard)
