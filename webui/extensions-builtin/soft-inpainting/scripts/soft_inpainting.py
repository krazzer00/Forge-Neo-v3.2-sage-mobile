...
def apply_adaptive_masks(settings: "SoftInpaintingSettings", nmask: torch.Tensor, latent_orig: torch.Tensor, latent_processed: torch.Tensor, overlay_images: list[Image.Image], width: int, height: int, paste_to: tuple[int]) -> list[Image.Image]:
    ...
    for i, (distance_map, overlay_image) in enumerate(zip(latent_distance, overlay_images)):
        ...
        converted_mask = weighted_histogram_filter(converted_mask, kernel, kernel_center, percentile_min=0.9, percentile_max=1, min_width=1)
        converted_mask = weighted_histogram_filter(converted_mask, kernel, kernel_center, percentile_min=0.25, percentile_max=0.75, min_width=1)
        ...
        image_masked.paste(overlay_image.convert("RGBA").convert("RGBa"), mask=ImageOps.invert(converted_mask.convert("L")))
...
def weighted_histogram_filter(img, kernel, kernel_center, percentile_min=0.0, percentile_max=1.0, min_width=1.0, n_jobs=-1):
    ...
    results = Parallel(n_jobs=n_jobs, backend="loky")(delayed(process_chunk)(chunk) for chunk in pixel_chunks)  # loky is fastest in my configuration
...
class Script(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        ...
        with InputAccordion(False, label=enabled_ui_label, elem_id=enabled_el_id) as soft_inpainting_enabled:
            with gr.Group():
                ...
                detail = gr.Slider(label=ui_labels.inpaint_detail_preservation, info=ui_info.inpaint_detail_preservation, minimum=1, maximum=32, step=0.5, value=default.inpaint_detail_preservation, elem_id=el_ids.inpaint_detail_preservation)

                gr.Markdown("### Pixel Composite Settings")

                mask_inf = gr.Slider(label=ui_labels.composite_mask_influence, info=ui_info.composite_mask_influence, minimum=0, maximum=1, step=0.05, value=default.composite_mask_influence, elem_id=el_ids.composite_mask_influence)
                dif_thresh = gr.Slider(label=ui_labels.composite_difference_threshold, info=ui_info.composite_difference_threshold, minimum=0, maximum=8, step=0.25, value=default.composite_difference_threshold, elem_id=el_ids.composite_difference_threshold)
        ...
        return [soft_inpainting_enabled, power, scale, detail, mask_inf, dif_thresh, dif_contr]
    ...
    def post_sample(self, p, ps: scripts.PostSampleArgs, enabled, power, scale, detail_preservation, mask_inf, dif_thresh, dif_contr):
        ...
        if getattr(ps.samples, "already_decoded", False):
            self.masks_for_overlay = apply_masks(settings=settings, nmask=nmask, overlay_images=self.overlay_images, width=p.width, height=p.height, paste_to=p.paste_to)
        else:
            self.masks_for_overlay = apply_adaptive_masks(settings=settings, nmask=nmask, latent_orig=p.init_latent, latent_processed=ps.samples, overlay_images=self.overlay_images, width=p.width, height=p.height, paste_to=p.paste_to)
