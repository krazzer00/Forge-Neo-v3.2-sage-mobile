...
def image_to_prompt(image, mode, clip_model_name):
    ...
    try:
        # Заменяем lowvram логику на совместимую с NeoForge
        if getattr(shared.cmd_opts, 'lowvram', False) or getattr(shared.cmd_opts, 'medvram', False):
            # В NeoForge используем стандартные методы управления памятью
            devices.torch_gc()
...
# 🔄 РАБОЧАЯ ФУНКЦИЯ КОПИРОВАНИЯ
def copy_prompt_with_pyperclip(prompt):
    """Копирование промпта с использованием pyperclip"""
    if not prompt or prompt in ["Ran out of VRAM", "Exception"]:
        return "❌ No valid prompt to copy"
...
def prompt_tab():
    with gr.Column():
        with gr.Row():
            # Фиксированное окно загрузки изображения 400x400
            image = gr.Image(
                type='pil', 
                label="Image", 
                height=400,
        ...
        # 🔄 ГРУППА COPY & SEND С КНОПКОЙ COPY PROMPT
        with gr.Group():
            gr.Markdown("### 📋 Copy & Send")
            
            with gr.Row():
                copy_btn = gr.Button("📋 Copy Prompt", variant='secondary')
            
            copy_status = gr.Textbox(label="Status", interactive=False, value="Ready")
    
    # Стандартные кнопки отправки
    with gr.Row():
        buttons = parameters_copypaste.create_buttons(["txt2img", "img2img", "inpaint", "extras"])
    
    # Обработчики событий
    generate_btn.click(image_to_prompt, inputs=[image, mode, clip_model], outputs=prompt)
    unload_button.click(unload)
    
    # 🔄 ОБРАБОТЧИК КОПИРОВАНИЯ
    copy_btn.click(copy_prompt_with_pyperclip, inputs=[prompt], outputs=copy_status)

    # Стандартная система отправки
...
def add_tab():
    global low_vram
    # Обновляем логику определения low_vram для NeoForge
    low_vram = getattr(shared.cmd_opts, 'lowvram', False) or getattr(shared.cmd_opts, 'medvram', False)
