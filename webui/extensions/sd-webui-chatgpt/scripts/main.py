...
try:
    # Renamed in A1111 1.6 / Forge; the old name is gone in Forge-Neo.
    from modules import infotext_utils as params_copypaste
except ImportError:
    from modules import generation_parameters_copypaste as params_copypaste

from modules.paths_internal import extensions_dir
from scripts import chatgptapi, localapi
...
_default_script_args = None


def build_default_script_args():
    """Default value of every txt2img script argument, as the UI would send them.

    Built once per process and cached, the same way modules/api/api.py does it -
    every call re-runs each extension's ui() function, which is both slow and
    unsafe to repeat while a generation is in flight.
    """
    global _default_script_args
    if _default_script_args is not None:
        return list(_default_script_args)
    ...
    with gr.Blocks():
        for script in modules.scripts.scripts_txt2img.scripts:
            try:
                elements = script.ui(False)
            except Exception as error:
                print(f'[sd-webui-chatgpt] skipping script args for {script}: {error}')
                elements = None
            if elements:
                script_args[script.args_from:script.args_to] = [getattr(elem, 'value', None) for elem in elements]

    _default_script_args = script_args
    return list(script_args)


def run_txt2img(txt2img_params):
    """Generate one image from the JSON preset.

    The preset is mapped straight onto StableDiffusionProcessingTxt2Img instead of
    being squeezed through the txt2img() signature, which changes between WebUI
    forks. Forge also runs all major T2I work on one dedicated thread, so the call
    is handed to modules_forge.main_thread rather than run inline.
    """
    import dataclasses
    ...
    params = copy.deepcopy(txt2img_params)

    # Presets written for older WebUI versions store names under *_index keys.
    sampler_index = params.pop('sampler_index', None)
    if isinstance(sampler_index, str) and sampler_index:
        params.setdefault('sampler_name', sampler_index)
    hr_sampler_index = params.pop('hr_sampler_index', None)
    if isinstance(hr_sampler_index, str) and hr_sampler_index:
        params.setdefault('hr_sampler_name', hr_sampler_index)

    styles = params.pop('prompt_styles', None) or params.get('styles') or []
    params['styles'] = [style for style in styles if style]
    override_settings_texts = params.pop('override_settings_texts', '')

    accepted = {f.name for f in dataclasses.fields(processing.StableDiffusionProcessingTxt2Img) if f.init}
    ignored = sorted(set(params) - accepted)
    if ignored:
        print('[sd-webui-chatgpt] ignoring unknown txt2img settings: ' + ', '.join(ignored))

    kwargs = {key: value for key, value in params.items() if key in accepted and value is not None}
    kwargs['outpath_samples'] = opts.outdir_samples or opts.outdir_txt2img_samples
    kwargs['outpath_grids'] = opts.outdir_grids or opts.outdir_txt2img_grids
    kwargs['override_settings'] = params_copypaste.create_override_settings_dict(override_settings_texts)

    script_args = build_default_script_args()

    def work():
        p = processing.StableDiffusionProcessingTxt2Img(**kwargs)
    ...
    processed = main_thread.run_and_wait_result(work)
    if processed is None or not processed.images:
        raise RuntimeError('txt2img produced no image, see the console for details.')
...
def init_or_change_backend(apikey, chatgpt_settings):
    ...
    backend = chatgpt_settings.get('backend', 'OpenAI API')

    if backend == 'OpenAI API':
        if type(chat_gpt_api) is chatgptapi.ChatGptApi:
            chat_gpt_api.change_apikey(apikey)
            chat_gpt_api.change_model(chatgpt_settings.get('model'))
            chat_gpt_api.change_base_url(chatgpt_settings.get('base_url'))
        else:
            chat_gpt_api = chatgptapi.ChatGptApi(
                chatgpt_settings.get('model'), apikey, chatgpt_settings.get('base_url'))
    elif backend == 'Ollama':
        if type(chat_gpt_api) is localapi.LocalChatApi:
            chat_gpt_api.load_settings(**chatgpt_settings)
        else:
            chat_gpt_api = localapi.LocalChatApi(**chatgpt_settings)
    else:
        # LlamaCpp / GPT4All cannot run on this build; report it instead of crashing.
        if type(chat_gpt_api) is localapi.UnavailableApi:
            chat_gpt_api.load_settings(**chatgpt_settings)
        else:
            chat_gpt_api = localapi.UnavailableApi(**chatgpt_settings)
...
def on_ui_tabs():
    ...
    if settings_file_path is not None:
        ...
        chatgpt_settings = { "model": "gpt-4o-mini", "backend": "OpenAI API" }
    ...
    def chatgpt_txt2img(request_prompt: str):
        ...
        if txt2img_params.get('prompt'):
            txt2img_params['prompt'] += ', ' + request_prompt
        else:
            txt2img_params['prompt'] = request_prompt

        images, info_js, info_html, comments_html = run_txt2img(txt2img_params)
    ...
    def chatgpt_txt2img_safe(request_prompt: str):
        global txt2img_error
        try:
            chatgpt_txt2img(request_prompt)
        except Exception as error:
            import traceback
            traceback.print_exc()
            txt2img_error = f'{type(error).__name__}: {error}'

    def append_chat_history(chat_history, text_input_str, result, prompt):
        global last_image_name, chat_history_images, txt2img_thread, txt2img_error
        if prompt is not None and prompt != '':
            if txt2img_thread is None:
                chatgpt_txt2img_safe(prompt)
            else:
                txt2img_thread.join()
            if txt2img_error is not None:
                failed = (result or '') + '\n\n**Image generation failed**\n\n`' + txt2img_error + '`'
                txt2img_error = None
                chat_history.append((text_input_str, failed))
                return chat_history
    ...
    def recv_stream(chat_history):
        ...
        while not stop_recv_thread:
            ...
            if prompt is not None and prompt != '' and txt2img_thread is None:
                txt2img_thread = threading.Thread(target=chatgpt_txt2img_safe, args=(prompt, ))
    ...
    def chatgpt_generate(chat_history):
        ...
        global txt2img_error, last_image_name
    ...
    with gr.Blocks(analytics_enabled=False) as runner_interface:
        ...
        with gr.Tabs() as setting_part_tabs:
            with gr.TabItem('OpenAI API', id='OpenAI API') as openai_api_tab_item:
                ...
                with gr.Row():
                    txt_base_url = gr.Textbox(value='', label='Base URL (any OpenAI-compatible server)',
                        placeholder=chatgptapi.DEFAULT_BASE_URL)
                    btn_base_url_save = gr.Button(value='Save And Reflect', variant='primary')
                    def base_url_save(setting_base_url: str):
                        chatgpt_settings['base_url'] = setting_base_url
                        with open(get_path_settings_file('chatgpt_settings.json'), 'w') as f:
                            json.dump(chatgpt_settings, f)
                        chat_gpt_api.change_base_url(setting_base_url)
                    btn_base_url_save.click(fn=base_url_save, inputs=txt_base_url)
            with gr.TabItem('Ollama', id='Ollama') as ollama_tab_item:
                with gr.Row():
                    ollama_model = gr.Textbox(label='Model Name')
                    ollama_url = gr.Textbox(label='Server URL', placeholder=localapi.DEFAULT_OLLAMA_URL)
                with gr.Row():
                    with gr.Column():
            ...
            with gr.TabItem('GPT4All', id='GPT4All') as gpt4all_tab_item:
                with gr.Row():
                    ...
                    with gr.Column():
                        gpt4all_prompt_template = gr.Textbox(label='Prompt Template')
                    with gr.Column():
                        btn_gpt4all_save = gr.Button(value='Save And Reflect', variant='primary')
                    def gpt4all_model_save(path: str, prompt_template: str):
                        chatgpt_settings['gpt4all_model'] = path
                        chatgpt_settings['gpt4all_prompt_template'] = prompt_template
                        with open(get_path_settings_file('chatgpt_settings.json'), 'w') as f:
                            json.dump(chatgpt_settings, f)
                        chat_gpt_api.load_settings(**chatgpt_settings)
                    btn_gpt4all_save.click(fn=gpt4all_model_save, inputs=[gpt4all_model_file, gpt4all_prompt_template])

        def ollama_save(name: str, url: str, n_gpu: int, n_ctx: int, keep_alive: str, system_message_language: str):
            chatgpt_settings['ollama_model'] = name
            chatgpt_settings['ollama_url'] = url
        ...
        btn_ollama_save.click(fn=ollama_save, inputs=[ollama_model, ollama_url, ollama_n_gpu, ollama_n_ctx, ollama_keep_alive, ollama_system_message_language],
        ...
        set_interactive_items = [text_input, btn_generate, btn_regenerate,
            ...
            txt_base_url, btn_base_url_save, ollama_url,
        ...
        def on_load():
            ...
            ret = [apikey, chatgpt_settings.get('model', 'gpt-4o-mini'), json_settings, setting_part_tabs_out, save_file_path,
            ...
            for key in ['llama_cpp_model', 'gpt4all_model', 'llama_cpp_full_template', 'llama_cpp_human_template', 'llama_cpp_ai_template', 'llama_cpp_system_message_language', 'gpt4all_prompt_template', 'llama_cpp_system_message_language', 'ollama_model', 'base_url', 'ollama_url']:
        ...
        runner_interface.load(on_load, outputs=[txt_apikey, txt_chatgpt_model, txt_json_settings, setting_part_tabs, txt_file_path,
            ...
            ollama_system_message_language, ollama_model, txt_base_url, ollama_url])
