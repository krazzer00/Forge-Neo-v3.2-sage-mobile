...
try:
    ...
    # Forge / Forge-Neo: checkpoint (re)loading is driven by the backend itself
    load_model, reload_model_weights = None, None
...
def webui_reload_model_weights(sd_model=None, info=None):
    if reload_model_weights is None and load_model is None:
        return
    try:
...
def backup_reload_ckpt_info(sem, info):
    ...
    with sem:
        ...
        if shared.sd_model is not None:
            backup_ckpt_info = getattr(shared.sd_model, "sd_checkpoint_info", None)
